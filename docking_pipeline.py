...
================================================================================
  ...
  ------------------------------------------------------------------------------
  Implements the full 5-stage structure-based docking workflow end-to-end
  with one command:
    ...
    Stage 3 - Ligand preparation
              accepts: SMILES string | .smi | .sdf | .mol2 | .pdb | .mol
    ...
    Stage 5 - Visualization & Report
              ...
              H-bond / hydrophobic interaction analysis, labeled pocket residues
              ReportLab -> single PDF report per run + global summary.csv

  Input modes (any may be mixed and matched freely):
    * Single        :   --protein <X> --ligand <Y>
    * CSV-driven    :   --csv pairs.csv                      (parallel rows)
    * Folder x Folder:  --protein-dir P/ --ligand-dir L/      (parallel pairs)

  Ligand input accepts, per-job, ANY of: SMILES string, .sdf, .mol2, .pdb, .mol,
  or a .smi file - auto-detected, no flag required.

  Author : Dr. VishDOCK Engine
  Python : 3.10+
================================================================================
"""
...
class HackerUI:
    ...
    def banner(self):
        if not self.enabled:
            print("=" * 72)
            print("  D R .   V I S H D O C K   --  Protein-Ligand Docking Suite")
        ...
        for i, line in enumerate(_VISHDOCK_BANNER.strip("\n").splitlines()):
        ...
        body = Group(
            *gradient_lines,
            ...
            Text(""),
            Align.center(Text(
                f"⚡ v{PIPELINE_VERSION}  •  ⟨ Computational Chemistry Engine ⟩  •  © Dr. VishDOCK",
...
def setup_logger(out_dir: Path, name: str = "docking", verbose: bool = True) -> logging.Logger:
    ...
    sh.setFormatter(fmt)
    # When the rich UI is active we route status through the UI itself; keep
    # the stream handler at ERROR so progress bar/panels are not interrupted
    # by INFO/WARNING lines (they still go to the on-disk log file).
    sh.setLevel(logging.ERROR if RICH else logging.INFO)
...
_KNOWN_PROTEIN_EXTS = {".pdb", ".ent", ".fasta", ".fa", ".faa", ".seq"}
_KNOWN_LIGAND_EXTS  = {".sdf", ".mol2", ".pdb", ".mol", ".smi", ".smiles"}
...
def detect_protein_input(value: str) -> str:
    ...
    # If the string LOOKS like a file path (has a recognizable protein-file
    # extension) but does not exist, fail with a clear, actionable message
    # instead of silently falling through and misclassifying the literal
    # path text as a sequence/ID (which produces a confusing downstream error).
    if p.suffix.lower() in _KNOWN_PROTEIN_EXTS:
        raise FileNotFoundError(
            f"Protein file not found: {value!r}  "
            f"(check the path is correct relative to your current working directory)"
        )
...
def detect_ligand_input(value: str) -> str:
    ...
    # Same path-vs-SMILES disambiguation as detect_protein_input above:
    # a missing "ligand.pdb" must NOT silently become the literal SMILES
    # string "ligand.pdb" (which happens to satisfy the SMILES character
    # set and produces a baffling "Invalid SMILES" error downstream).
    # NOTE: we deliberately do NOT treat "/" or "\" as path signals here -
    # both characters are legitimate SMILES cis/trans stereo-bond notation
    # (e.g. "C/C=C/C"), so that heuristic would misfire on valid input.
    if p.suffix.lower() in _KNOWN_LIGAND_EXTS:
        raise FileNotFoundError(
            f"Ligand file not found: {value!r}  "
            f"(check the path is correct relative to your current working directory)"
        )
...
def resolve_path_like(value: str, base_dir: Path) -> str:
    """
    If `value` looks like a filesystem path (has a recognizable protein- or
    ligand-file extension) but does not exist relative to the current
    working directory, try resolving it relative to `base_dir` (e.g. the
    folder containing a CSV jobs file). This lets a CSV/jobs file use paths
    relative to *itself* rather than forcing the user to always invoke the
    pipeline from one specific directory. IDs, SMILES, and raw sequences are
    returned unchanged - they are never path-resolved.
    """
    if Path(value).exists():
        return value
    suf = Path(value).suffix.lower()
    if suf in _KNOWN_PROTEIN_EXTS or suf in _KNOWN_LIGAND_EXTS:
...
class ProteinPreparer:
    ...
    def resolve(self, value: str) -> Path:
        ...
        if kind == "pdb_file":
            ...
            ok = self._fetch_alphafold(value.upper(), raw)
            if not ok:
                self.log.warning("AlphaFold DB miss; trying ESMFold fallback")
                ...
                if len(seq) > 400:
                    raise RuntimeError(
                        f"UniProt {value} is not in AlphaFold DB and its sequence "
                        f"({len(seq)} aa) exceeds the ESMFold public API limit (~400 aa). "
                        f"Please provide a local .pdb file or a PDB ID for this target instead."
                    )
    ...
    def _fetch_alphafold(self, uid: str, dest: Path) -> bool:
        """
        Robust AlphaFold DB fetch.

        AF DB has been progressively bumped (v2 → v3 → v4 → v5 → v6 …) and the
        version baked into the static URL changes with each release. A single
        hard-coded URL becomes stale. Strategy here:

          1. Hit the AlphaFold prediction API:
                 https://alphafold.ebi.ac.uk/api/prediction/{accession}
             which returns the *current* pdbUrl regardless of version.
          2. If the API is unreachable, walk static URLs v6 → v2 in turn.
        """
        api_url = f"https://alphafold.ebi.ac.uk/api/prediction/{uid}"
        self.log.info(f"Querying AlphaFold prediction API for {uid}")
        try:
            req = urllib.request.Request(api_url, headers={"User-Agent": "DockingPipeline/2.0"})
            ...
            for entry in entries:
                ...
                if pdb_url:
                    try:
                        http_download(pdb_url, dest, retries=2)
                        self.log.info(f"AlphaFold model fetched via API: {pdb_url}")
                        return True
                    except Exception as e:
                        self.log.debug(f"API-provided URL failed ({e}); trying next.")
        except Exception as e:
            self.log.debug(f"AlphaFold API unreachable: {e}")

        # Static fallback - newest version first
        for v in (6, 5, 4, 3, 2):
            url = f"https://alphafold.ebi.ac.uk/files/AF-{uid}-F1-model_v{v}.pdb"
            try:
                http_download(url, dest, retries=2)
                self.log.info(f"AlphaFold model_v{v} fetched: {url}")
                return True
            except Exception:
                continue
        self.log.warning(f"AlphaFold DB: no model available for {uid}")
        return False
...
class LigandPreparer:
    ...
    Designed so the user can hand the pipeline ANY of these per job, with
    zero extra flags - the format is auto-detected from content/extension:
        * a SMILES string typed directly on the command line / in a CSV cell
        * a .smi file (first non-comment, non-blank line is used)
        * a .sdf file (first molecule record is used)
        * a .mol2 file
        * a .mol (MDL Molfile)
        * a .pdb ligand snippet (e.g. extracted HETATM coordinates)

    Every file format has a two-strategy fallback chain: RDKit's native
    reader is tried first; if it fails (or returns a mol with zero bonds -
    a tell-tale sign of a PDB/MOL2 file lacking bond records), Open Babel's
    distance-based bond-order perception is used as a second pass. This
    matters most for `.pdb` ligand files, which frequently have no CONECT
    records and therefore no bond information for RDKit to read directly.
    """
    ...
    def prepare(self, value: str, name: str = "ligand") -> Tuple[Path, Path]:
        ...
        try:
            if kind == "smiles":
                ...
                smi = self._first_smiles_from_smi_file(Path(value))
                mol = self._from_smiles(smi)
            else:
                mol = self._from_file(Path(value), kind)
        except Exception as e:
            raise RuntimeError(
                f"Ligand preparation failed for input ({kind}) {value!r}: {e}"
            ) from e
    ...
    @staticmethod
    def _first_smiles_from_smi_file(path: Path) -> str:
        """Read the first usable SMILES token from a .smi file, skipping
        ...
        for line in path.read_text(errors="ignore").splitlines():
            ...
            if not line or line.startswith("#"):
                continue
            return line.split()[0]
        raise ValueError(f"No usable SMILES found in file: {path}")
    ...
    def _from_smiles(self, smi: str):
        ...
        if mol is None:
            raise ValueError(
                f"Invalid SMILES: {smi!r}. Verify the string parses in a "
                f"SMILES validator, or supply the ligand as .sdf/.mol2/.pdb instead."
            )
        mol = self._keep_largest_fragment(mol)   # strip counter-ions / salts
        mol = Chem.AddHs(mol)
        return mol

    def _from_file(self, path: Path, kind: str):
        """
        Multi-strategy file reader with automatic fallback.

        Strategy 1: RDKit's native parser for the detected format.
        Strategy 2: Open Babel bond-order perception (re-read into RDKit
                    via an intermediate SDF). This is the critical fallback
                    for .pdb ligand files without CONECT records, and for
                    malformed/non-standard .mol2 or .sdf files.
        """
        from rdkit import Chem
        ...
        try:
            mol = self._rdkit_native_read(path, kind)
        except Exception as e:
            errors.append(f"RDKit native reader: {e}")
            mol = None

        if mol is None or mol.GetNumAtoms() == 0:
            try:
                ob_mol = self._openbabel_read(path, kind)
                if ob_mol is not None and ob_mol.GetNumAtoms() > 0:
                    mol = ob_mol
            except Exception as e:
                errors.append(f"Open Babel fallback: {e}")

        if mol is None or mol.GetNumAtoms() == 0:
            raise ValueError(
                f"Could not parse ligand file ({kind}): {path}\n"
                f"  Tried: {' | '.join(errors) if errors else 'no strategies succeeded'}"
            )

        mol = self._keep_largest_fragment(mol)   # drop crystallization ions/waters
        mol = Chem.AddHs(mol, addCoords=True)
        return mol

    def _rdkit_native_read(self, path: Path, kind: str):
        """RDKit's own format-specific reader. May return None or raise."""
        from rdkit import Chem
        if kind == "sdf":
            suppl = Chem.SDMolSupplier(str(path), removeHs=False, sanitize=True)
            mols = [m for m in suppl if m is not None]
            if len(mols) > 1:
                self.log.info(f"SDF contains {len(mols)} records; using the first.")
            return mols[0] if mols else None
        if kind == "mol":
            return Chem.MolFromMolFile(str(path), removeHs=False, sanitize=True)
        if kind == "pdb":
            # NOTE: RDKit's PDB reader infers bonds only from CONECT records
            # or simple distance heuristics; it frequently fails or yields
            # zero bonds for ligand-only PDB snippets. The caller falls back
            # to Open Babel automatically when that happens.
        ...
        raise ValueError(f"No native RDKit reader registered for kind={kind!r}")

    def _openbabel_read(self, path: Path, kind: str):
        """
        Open Babel fallback: reads the file in its native chemistry toolkit
        (which performs its own bond-order / aromaticity perception from
        atomic distances and valence rules - far more forgiving than RDKit
        for ligand-only PDB/MOL2 snippets), writes an intermediate SDF, then
        hands that back to RDKit so all downstream code sees a normal
        RDKit Mol regardless of the original file format.
        """
        from openbabel import pybel
        from rdkit import Chem

        ob_format = {"pdb": "pdb", "mol2": "mol2", "sdf": "sdf", "mol": "mol"}.get(kind, kind)
        mol_iter = pybel.readfile(ob_format, str(path))
        obmol = next(mol_iter, None)
        if obmol is None:
            raise ValueError(f"Open Babel found no molecule records in {path}")

        obmol.OBMol.AddHydrogens()
        tmp_sdf = self.work / f"_tmp_openbabel_{kind}.sdf"
        obmol.write("sdf", str(tmp_sdf), overwrite=True)

        mol = Chem.MolFromMolFile(str(tmp_sdf), removeHs=False, sanitize=True)
        if mol is None:
            # last resort: parse without sanitizing, then sanitize leniently
            mol = Chem.MolFromMolFile(str(tmp_sdf), removeHs=False, sanitize=False)
            if mol is not None:
                try:
                    Chem.SanitizeMol(mol, catchErrors=True)
                except Exception:
                    pass
        self.log.info(f"Ligand parsed via Open Babel bond-order perception ({kind} -> sdf)")
        return mol

    def _keep_largest_fragment(self, mol):
        """
        Some ligand inputs are multi-fragment: a SMILES with a counter-ion
        salt ("CC(=O)O.[Na+]"), or a PDB/MOL2 snippet that accidentally
        includes a nearby crystallographic ion or water alongside the real
        ligand. Keep only the fragment with the most heavy atoms - this is
        almost always the intended ligand.
        """
        from rdkit import Chem
        ...
        best = max(frags, key=heavy_count)
        dropped = len(frags) - 1
        self.log.info(
            f"Ligand input had {len(frags)} disconnected fragments "
            f"(salts/ions/waters?); kept the largest, discarded {dropped}."
        )
        return best
    ...
    @staticmethod
    def _validate_pdbqt(path: Path) -> bool:
        """A 'successful' conversion can still produce an empty or atom-less
        file if the underlying tool silently failed. Verify there's at least
        one ATOM/HETATM record before trusting the result."""
    ...
    def _sdf_to_pdbqt(self, sdf_in: Path, pdbqt_out: Path) -> None:
        ...
        try:
            ...
            if self._validate_pdbqt(pdbqt_out):
                self.log.info("Ligand PDBQT via Meeko")
                return
            self.log.debug("Meeko produced an empty/invalid PDBQT; falling back to OpenBabel.")
        except Exception as e:
        ...
        try:
            ...
            if self._validate_pdbqt(pdbqt_out):
                ...
                return
            self.log.debug("Open Babel (pybel) produced an empty/invalid PDBQT; trying CLI.")
        except Exception as e:
        ...
        if which("obabel"):
            ...
            if rc == 0 and self._validate_pdbqt(pdbqt_out):
                self.log.info("Ligand PDBQT via obabel CLI")
                return

        raise RuntimeError(
            "Could not produce a valid ligand PDBQT (install meeko or open-babel; "
            "verify the source molecule has at least one heavy atom)."
        )
...
def _run_one_job(args_blob: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Short one-liner on the console; full traceback only in the on-disk
        # log file via DEBUG, and the dedicated ERROR.txt.
...
class Pipeline:
    ...
    def _jobs_from_csv(self, csv_path: Path) -> List[JobSpec]:
        if pd is None:
            raise RuntimeError("pandas is required for --csv mode (pip install pandas).")
        ...
        try:
            ...
            raise SystemExit(f"Could not parse CSV ({csv_path}): {e}")
        except UnicodeDecodeError:
            # Retry with a more permissive encoding (common with Excel exports)
            df = pd.read_csv(csv_path, encoding="latin-1")

        if df.empty:
            raise SystemExit(f"CSV file has no data rows: {csv_path}")

        cols = {str(c).lower().strip(): c for c in df.columns}
        if "ligand" not in cols and "smiles" in cols:
            cols["ligand"] = cols["smiles"]   # allow a 'smiles' column as an alias
        if "protein" not in cols or "ligand" not in cols:
            raise SystemExit(
                "CSV must contain at least 'protein' and 'ligand' columns "
        ...
        for i, row in df.iterrows():
            raw_protein = row[cols["protein"]]
            raw_ligand  = row[cols["ligand"]]
            if pd.isna(raw_protein) or pd.isna(raw_ligand) or \
               not str(raw_protein).strip() or not str(raw_ligand).strip():
                self.log.warning(f"CSV row {i + 2}: missing protein/ligand value - skipped.")
                skipped += 1
                continue

            protein = resolve_path_like(str(raw_protein).strip(), base_dir)
            ligand  = resolve_path_like(str(raw_ligand).strip(),  base_dir)
            ...
            jobs.append(JobSpec(protein=protein, ligand=ligand, name=name, center=center, size=size))

        if skipped:
            self.log.warning(f"Skipped {skipped} CSV row(s) with missing data.")
        if not jobs:
            raise SystemExit(f"No valid jobs found in CSV after validation: {csv_path}")

        self.log.info(f"Loaded {len(jobs)} job(s) from CSV {csv_path}")
        return jobs
    ...
    def _jobs_from_dirs(self, protein_dir: Optional[str], ligand_dir: Optional[str], pair_mode: str) -> List[JobSpec]:
        ...
        if not p_dir.is_dir():
            raise SystemExit(f"--protein-dir is not a directory (or does not exist): {protein_dir}")
        if not l_dir.is_dir():
            raise SystemExit(f"--ligand-dir is not a directory (or does not exist): {ligand_dir}")

        def _scan(folder: Path, allowed_exts: set, kind_label: str) -> List[Path]:
            """List recognized files in `folder`, skipping hidden/junk/unsupported files with a warning."""
            kept: List[Path] = []
            skipped: List[str] = []
            for f in sorted(folder.iterdir()):
                if not f.is_file():
                    continue
                if f.name.startswith("."):           # .DS_Store, .gitkeep, dotfiles
                    continue
            ...
            if skipped:
                self.log.warning(
                    f"{kind_label} dir: ignored {len(skipped)} unsupported file(s): "
                    f"{', '.join(skipped[:8])}{' ...' if len(skipped) > 8 else ''}"
                )
            return kept

        prots = _scan(p_dir, _KNOWN_PROTEIN_EXTS, "Protein")
        ligs  = _scan(l_dir, _KNOWN_LIGAND_EXTS,  "Ligand")

        if not prots:
            raise SystemExit(
                f"No recognized protein files in {protein_dir} "
                f"(expected extensions: {', '.join(sorted(_KNOWN_PROTEIN_EXTS))})"
            )
        if not ligs:
            raise SystemExit(
                f"No recognized ligand files in {ligand_dir} "
                f"(expected extensions: {', '.join(sorted(_KNOWN_LIGAND_EXTS))})"
            )
        ...
        if pair_mode == "zip":
            ...
            if len(prots) != len(ligs):
                self.log.warning(
                    f"--pair-mode zip: protein/ligand count mismatch "
                    f"({len(prots)}/{len(ligs)}); using first {n} of each."
                )
        ...
        self.log.info(
            f"Folder mode ({pair_mode}): {len(prots)} protein(s) x {len(ligs)} ligand(s) -> {len(jobs)} job(s)"
        )
        return jobs
...
def build_cli() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description=(
            "Dr. VishDOCK - Advanced Automated Protein-Ligand Docking Pipeline. "
            "Full 5-stage structure-based docking workflow in one command."
        ),
...
--------
  ...
  # Ligand as .sdf / .mol2 / .pdb - all auto-detected, no extra flag needed
  python docking_pipeline.py --protein 1HSG --ligand ligand.sdf  -o out/
  python docking_pipeline.py --protein 1HSG --ligand ligand.mol2 -o out/
  python docking_pipeline.py --protein 1HSG --ligand ligand.pdb  -o out/
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    log.info(f"=== Dr. VishDOCK v{PIPELINE_VERSION} ===")
